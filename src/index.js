...
class OthelloGame {
  ...
  initGame() {
    ...
    if (!this.elm) throw new Error(`Element not found for id ${selector}`)
  ...
  createNutElm(type, validMove = false, turn) {
    ...
    nut.style.width = `${Math.floor(this.options.cellMaxWidth * 0.8)}px`
    nut.style.height = `${Math.floor(this.options.cellMaxWidth * 0.8)}px`
  ...
  handleCellClick(x, y) {
    return () => {
      if (this.board.finished) {
        ...
        console.log(result)
        // create a div and put the result in it
        const resultElm = document.createElement('div')
        resultElm.setAttribute("id", "result")
        resultElm.innerHTML = `Winner: ${result.winner} (${
          result[result.winner]
        })`
        if (result.white === result.black) resultElm.innerHTML = `Equal!`
        this.elm.appendChild(resultElm)
        return
      }
...
let cellMaxWidth = Math.min(
  100,
  (Math.min(window.innerWidth, window.innerHeight) - 100) / 8,
)
const OG = new OthelloGame({ cellMaxWidth })
